...
public class StorageController extends Job implements HeartListener {
	...
	public File createFile(Patient pat, String name)
			throws IOException {
		...
		if (Hub.localCfg.get(Preferences.PREF_DOSAVE, true) && dm != null && pat!=null) {
			Metafile mf = new Metafile(pat, name, category,
					System.currentTimeMillis(), dest);
			metafiles.add(mf);
		}
		return dest;
	}
	...
	private void addDocument(Metafile mf) throws Exception {
		...
		GenericDocument gd = new GenericDocument(mf.pat, mf.name, mf.category,
				mf.fileOnDisk, new TimeTool().toString(TimeTool.DATE_GER), "",
				null);
		dm.addDocument(gd);
		mf.guid = gd.getGUID();
