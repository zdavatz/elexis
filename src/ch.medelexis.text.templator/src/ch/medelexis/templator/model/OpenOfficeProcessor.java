	...
	public boolean doOutput(ProcessingSchema schema) {
		...
		if (!tmpl.exists()) {
			SWTHelper.alert("Template missing", MessageFormat.format(
					"Konnte Vorlagedatei {0} nicht öffnen",
					tmpl.getAbsolutePath()));
			return false;
		}
		try {
			...
			while ((ze = zis.getNextEntry()) != null) {
				...
				if (ze.getName().equals("content.xml")
						|| ze.getName().equals("styles.xml")) {
					SchemaFilterOutputStream sfo = new SchemaFilterOutputStream(
							proc, zos,this);
			...
			zos.close();
			zis.close();
			String cmd = Hub.localCfg.get(OOOProcessorPrefs.PREFERENCE_BRANCH
					+ "cmd", "swriter.exe");
			String param = Hub.localCfg.get(OOOProcessorPrefs.PREFERENCE_BRANCH
					+ "param", "%");
			int i = param.indexOf('%');
			if (i != -1) {
				param = param.substring(0, i) + output.getAbsolutePath()
						+ param.substring(i + 1);
			}
			/* Process process = */Runtime.getRuntime().exec(
					new String[] { cmd, param });
			return true;
		} catch (Exception e) {
			ExHandler.handle(e);
			SWTHelper
					.alert("OpenOffice Processor",
							"Problem mit dem Erstellen des Dokuments "
									+ e.getMessage());
		}
		return false;
	}
