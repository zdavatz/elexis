...
public class PerlTest extends TestCase {
	static String input = Util.loadFileToString("rsc/issue56-1.yaml");
...
public void testMaps(){
	...
	for (Object obj : yaml.loadAll(input)) {
		...
		if (oid == 123058)
		{
			...
			String shouldBe = ":de=Acetylsalicylsäure";
			System.out.println(String.format("%1$s %2$s %3$s", shouldBe, acidNameDe,
				shouldBe.equals(acidNameDe)));
			
			// Passes assertEquals(acidNameDe, ":de=AcetylsalicylsÃ¤ure");
			assertEquals(shouldBe, acidNameDe);
		}
