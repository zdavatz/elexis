// $Id: TimeTool.java 4425 2008-09-21 15:50:10Z rgw_ch $
		...
		try {
			...
			if (s1.length != 2) // kein Abstand
			{
				...
				if (r == null) // Nein. Ist es ein Datum-String?
				{
					...
					{
						if (s.length() == 14) {
							set(Integer.parseInt(s.substring(0, 4)), Integer.parseInt(s.substring(
								4, 6)) - 1, Integer.parseInt(s.substring(6, 8)), Integer.parseInt(s
								.substring(8, 10)), Integer.parseInt(s.substring(10, 12)), Integer
