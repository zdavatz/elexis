...
public class LocalApplicationWizardDefinePage extends WizardPage implements IWizardPage {
  ...
  private void constructHomeSection(FormToolkit formToolkit, ScrolledForm scrolledForm) {
    ...
    textHome.addModifyListener(new ModifyListener() {
      public void modifyText(ModifyEvent modifyEvent) {
        ...
        IApplicationInfo applicationInfo = selectApplicationInfo(textHome.getText());
        textHome.setFocus();
  ...
  private void constructApplicationsSection(FormToolkit formToolkit, ScrolledForm scrolledForm) {
    ...
    tableApplicationInfos.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent selectionEvent) {
        ...
        IApplicationInfo applicationInfo = (IApplicationInfo)tableItem.getData();
        if(applicationInfo != null) {
  ...
  private void checkPageState() {
    ...
    if(home.length() != 0) {
      ...
      if(file.canRead()) {
        try {
          IApplicationAssistant applicationAssistant = OfficeApplicationRuntime.getApplicationAssistant(EditorCorePlugin.getDefault().getLibrariesLocation());
          IApplicationInfo applicationInfo = applicationAssistant.findLocalApplicationInfo(home);
          if(applicationInfo == null) {
            setPageComplete(true);
            setMessage(Messages.LocalApplicationWizardDefinePage_message_warning_path_invalid, IMessageProvider.WARNING);
          }
          else {            
            if(applicationInfo.getMajorVersion() == 1 && applicationInfo.getMinorVersion() == 9) {
              setPageComplete(true);
              setMessage(Messages.LocalApplicationWizardDefinePage_message_warning_beta_release, IMessageProvider.WARNING);
            }
            else if(applicationInfo.getMajorVersion() == 1) {
              setPageComplete(true);
              setMessage(Messages.LocalApplicationWizardDefinePage_message_warning_version_old, IMessageProvider.WARNING);
            }
            else {
              setPageComplete(true);
              setMessage(null); 
        ...
        setMessage(Messages.LocalApplicationWizardDefinePage_message_error_path_not_available, IMessageProvider.ERROR);
      }
    }
    else {
      setPageComplete(false);
  ...
  private void fillTable(Table table) {
    ...
    for(int i=0, n=applicationInfos.length; i<n; i++) {
      IApplicationInfo applicationInfo = applicationInfos[i];
