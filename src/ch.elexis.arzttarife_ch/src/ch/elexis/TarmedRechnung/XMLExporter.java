...
public class XMLExporter implements IRnOutputter {
	...
	public Document doExport(final Rechnung rechnung, final String dest,
		...
		validator.setAttribute(ATTR_VERSION_SOFTWARE, vi.getMaior() + vi.getMinor()
			+ vi.getRevision()); // 10130
		...
		for (Konsultation b : lb) {
			...
			for (Verrechnet vv : lv) {
				...
				if (v instanceof TarmedLeistung) {
					...
					// gegen
					// 0)
					el.setAttribute(ATTR_SCALE_FACTOR_MT, XMLTool
						.doubleToXmlDouble(primaryScale, 1)); // 22490
					el.setAttribute(ATTR_EXTERNAL_FACTOR_MT, XMLTool.doubleToXmlDouble(
						secondaryScale, 1)); // 22500
					...
					el.setAttribute(ATTR_SCALE_FACTOR_TT, XMLTool
						.doubleToXmlDouble(primaryScale, 1)); // 22540
		...
		// 11402
		biller.setAttribute(ATTR_EAN_PARTY, TarmedRequirements.getEAN(actMandant
			.getRechnungssteller())); // 11402
		...
		if (gesetz.equalsIgnoreCase("ivg")) { //$NON-NLS-1$
			...
			if ((!caseNumber.matches("[0-9]{14}")) && // seit 1.1.2000 gültige Nummer //$NON-NLS-1$
				...
				if (Hub.userCfg.get(Leistungscodes.BILLING_STRICT, true)) {
					rn
						.reject(REJECTCODE.VALIDATION_ERROR,
		...
		if (rn.getStatus() != RnStatus.FEHLERHAFT) {
			try {
				...
				ExHandler.handle(ex);
				SWTHelper.alert(Messages.XMLExporter_ErrorCaption, MessageFormat.format(
