...
public class Leistungscodes extends PreferencePage implements IWorkbenchPreferencePage {
	...
	protected Control createContents(final Composite parent){
		...
		SWTHelper.createHyperlink(ret, Messages.Leistungscodes_delete, new HyperlinkAdapter() {
			...
			public void linkActivated(HyperlinkEvent e){
				...
				if (SWTHelper.askYesNo(MessageFormat.format(Messages.Leistungscodes_reallyDelete,
					bName), Messages.Leistungscodes_notUndoable)) {
		...
		protected Control createDialogArea(final Composite parent){
			...
			if (Hub.acl.request(AccessControlDefaults.CASE_DEFINE_SPECIALS) == true) {
				...
				fdUnused.addMoveToAction(ldRequirements,
					Messages.Leistungscodes_moveItemToRequiredData, Desk.IMG_MOVETOUPPERLIST, true);
				fdUnused.addMoveToAction(ldOptional,
					Messages.Leistungscodes_moveItemToOptionalData, Desk.IMG_MOVETOLOWERLIST, true);
	...
	class FieldDefsDisplay extends Composite {
		...
		public FieldDefsDisplay(Composite parent, int style, String[] listItems){
			...
			addToolItem(toolBar, Desk.IMG_ARROWUP, Messages.Leistungscodes_moveItemUp, MOVEITEMUP,
				true);
			addToolItem(toolBar, Desk.IMG_ARROWDOWN, Messages.Leistungscodes_moveItemDown,
				MOVEITEMDOWN, true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown, -1, false);
			addToolItem(toolBar, Desk.IMG_REMOVEITEM, Messages.Leistungscodes_deleteItem,
				DELETEITEM, true);
			addToolItem(toolBar, Desk.IMG_EDIT, Messages.Leistungscodes_editItem, EDITITEM, true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown, -1, false);
			
			// ****** create popupmenu for this list (same actions)
			ListPopUpMenuAction moveItemUpAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_moveItemUp, Desk.IMG_ARROWUP,
					Messages.Leistungscodes_moveItemUp, MOVEITEMUP, listDisplay, true);
			ListPopUpMenuAction moveItemDownAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_moveItemDown, Desk.IMG_ARROWDOWN,
					Messages.Leistungscodes_moveItemDown, MOVEITEMDOWN, listDisplay, true);
			ListPopUpMenuAction delItemAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_deleteAction, Desk.IMG_REMOVEITEM,
					Messages.Leistungscodes_removeConstraintTT, DELETEITEM, listDisplay, true);
			ListPopUpMenuAction changeItemAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_editItem, Desk.IMG_EDIT,
		...
		private org.eclipse.swt.widgets.List getListPart(ListDisplay<String> listDisplay){
			...
			for (int li = 0; li < children.length; li++) {
				...
				if (child.getClass().toString().equalsIgnoreCase(
					"class org.eclipse.swt.widgets.List")) { //$NON-NLS-1$
			...
			public void widgetSelected(SelectionEvent e){
				...
				default:
					if (type > LASTFIXEDITEM) {
						moveItemToOtherList(listDisplay, moveTo_DestinationLists.get(type
							- LASTFIXEDITEM - 1), noDuplicatesList);
					}
					break;
				}
			}
			...
			public void run(){
				...
				default:
					if (actionType > LASTFIXEDITEM) {
						moveItemToOtherList(listDisplay, moveTo_DestinationLists.get(actionType
							- LASTFIXEDITEM - 1), noDuplicatesList);
					}
					break;
				}
