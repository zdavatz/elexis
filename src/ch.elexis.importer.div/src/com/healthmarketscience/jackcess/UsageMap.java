...
public class UsageMap {
	...
	/**
	 * the current handler implementation for reading/writing the specific usage map type. note,
	...
	/**
	 * @param database
	 *            database that contains this usage map
	 * @param tableBuffer
	 *            Buffer that contains this map's declaration
	 * @param pageNum
	 *            Page number that this usage map is contained in
	 * @param rowStart
	 *            Offset at which the declaration starts in the buffer
	 */
	 ...
	 * @return Either an InlineUsageMap or a ReferenceUsageMap, depending on which type of map is
	 *         found
	 */
	public static UsageMap read(Database database, int pageNum, int rowNum,
		boolean assumeOutOfRangeBitsOn) throws IOException{
	...
	private void initHandler(byte mapType, boolean assumeOutOfRangeBitsOn) throws IOException{
	...
	protected int getNextPageNumber(int curPage){
		return bitIndexToPageNumber(getNextBitIndex(pageNumberToBitIndex(curPage)),
	...
	protected int getPrevPageNumber(int curPage){
		return bitIndexToPageNumber(getPrevBitIndex(pageNumberToBitIndex(curPage)),
			...
			if (b != (byte) 0) {
				for (int i = 0; i < 8; i++) {
					if ((b & (1 << i)) != 0) {
						...
						int pageNumber =
							bitIndexToPageNumber(pageNumberOffset, PageChannel.INVALID_PAGE_NUMBER);
						if (!isPageWithinRange(pageNumber)) {
							throw new IllegalStateException("found page number " + pageNumber
								+ " in usage map outside of expected range " + _startPage + " to "
								+ _endPage);
						}
	...
	protected void updateMap(int absolutePageNumber, int bufferRelativePageNumber,
		ByteBuffer buffer, boolean add) throws IOException{
		...
		if (isOn == add) {
			throw new IOException("Page number " + absolutePageNumber + " already "
				+ ((add) ? "added to" : "removed from") + " usage map, expected range "
	...
	public String toString(){
		StringBuilder builder =
			new StringBuilder("(" + _handler.getClass().getSimpleName() + ") page numbers (range "
				+ _startPage + " " + _endPage + "): [");
	...
	private abstract class Handler {
		...
		public boolean containsPageNumber(int pageNumber){
			return (isPageWithinRange(pageNumber) && getPageNumbers().get(
				pageNumberToBitIndex(pageNumber)));
		}
		
		/**
		 * @param pageNumber
		 *            Page number to add or remove from this map
		 * @param add
		 *            True to add it, false to remove it
		 */
		public abstract void addOrRemovePageNumber(int pageNumber, boolean add) throws IOException;
	}
	
	/**
	 * Usage map whose map is written inline in the same page. For Jet4, this type of map can
	 * usually contains a maximum of 512 pages. Free space maps are always inline, used space maps
	 * may be inline or reference. It has a start page, which all page numbers in its map are
	...
	private class InlineHandler extends Handler {
		...
		private InlineHandler(boolean assumeOutOfRangeBitsOn) throws IOException{
		...
		@Override
		public boolean containsPageNumber(int pageNumber){
			return (super.containsPageNumber(pageNumber) || (_assumeOutOfRangeBitsOn
				&& (pageNumber >= 0) && !isPageWithinRange(pageNumber)));
		}
		
		@Override
		public void addOrRemovePageNumber(int pageNumber, boolean add) throws IOException{
			if (isPageWithinRange(pageNumber)) {
				...
				if (add) {
					...
					if (_assumeOutOfRangeBitsOn) {
						...
						throw new IOException("Page number " + pageNumber
							+ " already removed from usage map" + ", expected range " + _startPage
							+ " to " + _endPage);
					}
				}
		 ...
		 * 
		 * @param newStartPage
		 *            new page at which to start
		 * @param newPageNumber
		 *            optional page number to add once the map has been shifted to the new start
		 *            page
		 */
		private void moveToNewStartPage(int newStartPage, int newPageNumber) throws IOException{
		...
		/**
		 * Shifts the inline usage map so that it now starts with the given firstPage (if valid),
		 * otherwise the newPageNumber. Any page numbers added to the end of the usage map are set
		 * to "on".
		 * 
		 * @param firstPage
		 *            current first used page
		 * @param newPageNumber
		 *            page number to remove once the map has been shifted to the new start page
		 */
	...
	/**
	 * Usage map whose map is written across one or more entire separate pages of page type
	 * USAGE_MAP. For Jet4, this type of map can contain 32736 pages per reference page, and a
	 * maximum of 17 reference map pages for a total maximum of 556512 pages (2 GB).
	 * 
	 * @author Tim McCune
	 */
	private class ReferenceHandler extends Handler {
		...
		private ReferenceHandler() throws IOException{
			...
			for (int i = 0; i < numUsagePages; i++) {
				...
				if (mapPageNum > 0) {
					ByteBuffer mapPageBuffer = _mapPageHolder.setPage(getPageChannel(), mapPageNum);
		...
		private int getMaxPagesPerUsagePage(){
			return ((getFormat().PAGE_SIZE - getFormat().OFFSET_USAGE_MAP_PAGE_DATA) * 8);
		}
		...
		public void addOrRemovePageNumber(int pageNumber, boolean add) throws IOException{
			...
			updateMap(pageNumber, (pageNumber - (getMaxPagesPerUsagePage() * pageIndex)),
				mapPageBuffer, add);
		 ...
		 * 
		 * @param pageIndex
		 *            Index of the page reference within the map declaration
		 */
	...
	/**
	 * Utility class to traverse over the pages in the UsageMap. Remains valid in the face of usage
	 * map modifications.
	 */
	public final class PageCursor {
		...
		/**
		 * the last read modification count on the UsageMap. we track this so that the cursor can
		 * detect updates to the usage map while traversing and act accordingly
		 */
		...
		/**
		 * Returns {@code true} if this cursor is up-to-date with respect to its usage map.
		 */
		...
		/**
		 * After calling this method, {@link #getPreviousPage} will return the last page in the map
		 */
