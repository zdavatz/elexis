...
/**
 * Encoding text data in Unicode often requires more storage than using an existing 8-bit character
 * set and limited to the subset of characters actually found in the text. The Unicode Compression
 * Algorithm reduces the necessary storage while retaining the universality of Unicode. A full
 * description of the algorithm can be found in document
 * http://www.unicode.org/unicode/reports/tr6.html
 * 
 * Summary
 * 
 * The goal of the Unicode Compression Algorithm is the abilty to Express all code points in Unicode
 * Approximate storage size for traditional character sets Work well for short strings Provide
 * transparency for Latin-1 data Support very simple decoders Support simple as well as
 * sophisticated encoders
 * 
 * If needed, further compression can be achieved by layering standard file or disk-block based
 * compression algorithms on top.
 * 
 * <H2>Features</H2>
 * 
 * Languages using small alphabets would contain runs of characters that are coded close together in
 * Unicode. These runs are interrupted only by punctuation characters, which are themselves coded in
 * proximity to each other in Unicode (usually in the ASCII range).
 * 
 * Two basic mechanisms in the compression algorithm account for these two cases, sliding windows
 * and static windows. A window is an area of 128 consecutive characters in Unicode. In the
 * compressed data stream, each character from a sliding window would be represented as a byte
 * between 0x80 and 0xFF, while a byte from 0x20 to 0x7F (as well as CR, LF, and TAB) would always
 * mean an ASCII character (or control).
 * 
...
public abstract class SCSU {
	...
	/**
	 * SQ<i>n</i> Quote from Window .
	 * <p>
	 * If the following byte is less than 0x80, quote from static window <i>n</i>, else quote from
	 * dynamic window <i>n</i>.
	 */
	...
	/**
	 * SC<i>n</i> Change to Window <i>n</i>.
	 * <p>
	 * If the following bytes are less than 0x80, interpret them as command bytes or pass them
	 * through, else add the offset for dynamic window <i>n</i>.
	 */
	...
	int dynamicOffset[] =
		{
			initialDynamicOffset[0], initialDynamicOffset[1], initialDynamicOffset[2],
			initialDynamicOffset[3], initialDynamicOffset[4], initialDynamicOffset[5],
			initialDynamicOffset[6], initialDynamicOffset[7]
		};
	...
	/**
	 * Unicode code points from 3400 to E000 are not adressible by dynamic window, since in these
	 * areas no short run alphabets are found. Therefore add gapOffset to all values from
	 * gapThreshold
	 */
