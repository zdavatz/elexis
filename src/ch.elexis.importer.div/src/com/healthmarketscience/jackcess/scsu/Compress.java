 ...
 **/
/*
 * Note on exception handling This compressor is designed so that it can be restarted after an
 * exception. All operations advancing input and/or output cursor (iIn and iOut) either complete an
 * action, or set a state (fUnicodeMode) before updating the cursors.
 */
public class Compress extends SCSU {
	 ...
	 * the ASCII range, but quote characters overlapping with compression command codes. Runs of
	 * characters fitting the current window are output as runs of bytes in the range 0x80-0xFF.
	 * Checks for and validates Surrogate Pairs. Uses and updates the current input and output
	 * cursors store in the instance variables <i>iIn</i> and <i>iOut</i>.
	 * 
	 * @param in
	...
	public int outputSingleByteRun(char[] in, byte[] out) throws EndOfOutputException,
		EndOfInputException, IllegalInputException{
		int iWin = getCurrentWindow();
		while (iIn < in.length) {
			int outlen = 0;
			byte byte1 = 0;
	...
	/**
	 * quote a single character in single byte mode Quoting a character (aka 'non-locking shift')
	 * gives efficient access to characters that occur in isolation--usually punctuation characters.
	 * When quoting a character from a dynamic window use 0x80 - 0xFF, when quoting a character from
	 * a static window use 0x00-0x7f.
	 * 
	 * @param ch
	 *            - character to be quoted
	 * @param out
	 *            - output byte array
	 **/
	...
	private void quoteSingleByte(int ch, byte[] out) throws EndOfOutputException{
		...
		// Letter that fits the current static window
			...
			throw new IllegalStateException("ch = " + ch
				+ " not valid in quoteSingleByte. Internal Compressor Error");
		}
		// advance input pointer
		iIn++;
	 ...
	 * characters. Characters in the range 0xE00-0xF2FF must be quoted to avoid overlap with the
	 * Unicode mode compression command codes. Uses and updates the current input and output cursors
	 * store in the instance variables <i>iIn</i> and <i>iOut</i>. NOTE: Characters from surrogate
	 * pairs are passed through and unlike single byte mode no checks are made for unpaired
	...
	/**
	 * redefine a window so it surrounds a given character value For now, this function uses window
	 * 3 exclusively (window 4 for extended windows);
	 * 
	 * @return true if a window was successfully defined
	 * @param ch
	 *            - character around which window is positioned
	 * @param out
	 *            - output byte array
	 * @param fCurUnicodeMode
	 *            - type of window
	 **/
