...
public class IndexPageCache {
	...
	private final Map<Integer, DataPageMain> _dataPages = new HashMap<Integer, DataPageMain>();
	...
	/**
	 * Handles any modified pages which are empty as the first pass during a {@link #write} call.
	 * All empty pages are removed from the _modifiedPages collection by this method.
	 */
	...
	/**
	 * Prepares any non-empty modified pages for writing as the second pass during a {@link #write}
	 * call. Updates entry prefixes, promotes/demotes tail pages, and splits pages as needed.
	 */
	...
	/**
	 * Writes any non-empty modified pages as the last pass during a {@link #write} call. Clears the
	 * _modifiedPages collection when finised.
	 */
	...
	/**
	 * Returns a CacheDataPage for the given page number, may be {@code null} if the given page
	...
	/**
	 * Returns a DataPageMain for the given page number, may be {@code null} if the given page
	 * number is invalid. Loads the given page if necessary.
	 */
	 ...
	 * 
	 * @param cacheDataPage
	 *            the page from which to remove the entry
	 * @param entryIdx
	 *            the index of the entry to remove
	 */
	private void removeEntry(CacheDataPage cacheDataPage, int entryIdx) throws IOException{
	 ...
	 * 
	 * @param cacheDataPage
	 *            the page to which to add the entry
	 * @param entryIdx
	 *            the index at which to add the entry
	 * @param newEntry
	 *            the entry to add
	 */
	private void addEntry(CacheDataPage cacheDataPage, int entryIdx, Entry newEntry)
		throws IOException{
	 ...
	 *            the page to update
	 * @param entryIdx
	 *            the index at which to add/remove/replace the entry
	 * @param newEntry
	 *            the entry to add/replace
	 * @param upType
	 *            the type of update to make
	 */
	private void updateEntry(CacheDataPage cacheDataPage, int entryIdx, Entry newEntry,
		UpdateType upType) throws IOException{
		DataPageMain dpMain = cacheDataPage._main;
		...
		CacheDataPage parentDataPage =
			(!dpMain.isRoot() ? new CacheDataPage(dpMain.getParentPage()) : null);
	 ...
	 * 
	 * @param parentDataPage
	 *            the parent of the removed page
	 * @param cacheDataPage
	 *            the page to remove
	 * @param oldLastEntry
	 *            the last entry for this page (before it was removed)
	 */
	private void removeDataPage(CacheDataPage parentDataPage, CacheDataPage cacheDataPage,
		Entry oldLastEntry) throws IOException{
		DataPageMain dpMain = cacheDataPage._main;
		DataPageExtra dpExtra = cacheDataPage._extra;
		...
		if (dpExtra._totalEntrySize != 0) {
			throw new IllegalStateException("Empty page but size is not 0? "
				+ dpExtra._totalEntrySize + ", " + cacheDataPage);
		}
	 ...
	 * 
	 * @param parentDataPage
	 *            the parent page to which to add the entry
	 * @param childDataPage
	 *            the child from which to get the entry to add
	 */
	 ...
	 * 
	 * @param parentDataPage
	 *            the parent page in which to replace the entry
	 * @param childDataPage
	 *            the child for which the entry is being replaced
	 * @param oldEntry
	 *            the old child entry for the child page
	 */
	private void replaceParentEntry(CacheDataPage parentDataPage, CacheDataPage childDataPage,
		Entry oldEntry) throws IOException{
		DataPageExtra childExtra = childDataPage._extra;
		updateParentEntry(parentDataPage, childDataPage, oldEntry, childExtra._entryView.getLast(),
			UpdateType.REPLACE);
	}
	
	/**
	 * Updates the entry for the given child page in the given parent page according to the given
	 * updateType.
	 * 
	 * @param parentDataPage
	 *            the parent page in which to update the entry
	 * @param childDataPage
	 *            the child for which the entry is being updated
	 * @param oldEntry
	 *            the old child entry to remove/replace
	 * @param newEntry
	 *            the new child entry to replace/add
	 * @param upType
	 *            the type of update to make
	 */
	private void updateParentEntry(CacheDataPage parentDataPage, CacheDataPage childDataPage,
		Entry oldEntry, Entry newEntry, UpdateType upType) throws IOException{
		...
		if (idx < 0) {
			if (expectFound) {
				throw new IllegalStateException("Could not find child entry in parent; childEntry "
			...
			if (!expectFound) {
				throw new IllegalStateException(
					"Unexpectedly found child entry in parent; childEntry " + newEntry
	 ...
	 * 
	 * @param parentDataPage
	 *            the parent page in which to update the child tail
	 * @param childDataPage
	 *            the child to add/replace
	 * @param upType
	 *            the type of update to make
	 */
	private void updateParentTail(CacheDataPage parentDataPage, CacheDataPage childDataPage,
		UpdateType upType) throws IOException{
		DataPageMain parentMain = parentDataPage._main;
		
		int newChildTailPageNumber =
			((upType == UpdateType.REMOVE) ? INVALID_INDEX_PAGE_NUMBER
	 ...
	 * 
	 * @param dpMain
	 *            the page to which the entry will be added
	 * @param entry
	 *            the entry being added
	 * @throws IllegalStateException
	 *             if the entry type does not match the page type
	 */
	private void validateEntryForPage(DataPageMain dpMain, Entry entry){
		if (dpMain._leaf != entry.isLeafEntry()) {
			throw new IllegalStateException(
				"Trying to update page with wrong entry type; pageLeaf " + dpMain._leaf
					+ ", entryLeaf " + entry.isLeafEntry());
		}
	}
	...
	private void splitDataPage(CacheDataPage origDataPage) throws IOException{
		...
		CacheDataPage newDataPage =
			allocateNewCacheDataPage(parentMain._pageNumber, origMain._leaf);
	 ...
	 * This must be done when the root page needs to be split.
	 * 
	 * @param rootDataPage
	 *            the root data page
	 * 
	 ...
	 * 
	 * @param parentPageNumber
	 *            the parent page for the new page
	 * @param isLeaf
	 *            whether or not the new page is a leaf page
	 * 
	 * @return the newly created page
	 */
	private CacheDataPage allocateNewCacheDataPage(Integer parentPageNumber, boolean isLeaf)
		throws IOException{
	 ...
	 * 
	 * @param cacheDataPage
	 *            the index page to be separated
	 */
	 ...
	 * 
	 * @param cacheDataPage
	 *            the page whose children need to be updated
	 */
	private void reparentChildren(CacheDataPage cacheDataPage) throws IOException{
		...
		for (Entry entry : dpExtra._entryView) {
			...
			if (childMain != null) {
				childMain.setParentPage(dpMain._pageNumber, dpMain
	...
	/**
	 * Makes the tail entry of the given page a normal entry on that page, done when there is only
	 * one entry left on a page, and it is the tail.
	 * 
	 * @param cacheDataPage
	 *            the page whose tail must be updated
	 */
	...
	/**
	 * Makes the last normal entry of the given page the tail entry on that page, done when there
	 * are multiple entries on a page and no tail entry.
	 * 
	 * @param cacheDataPage
	 *            the page whose tail must be updated
	 */
	...
	/**
	 * Marks the given index page as modified and saves it for writing, if necessary (if the page is
	 * already marked, does nothing).
	 * 
	 * @param cacheDataPage
	...
	private void validateEntries(DataPageExtra dpExtra) throws IOException{
		...
		if (entrySize != dpExtra._totalEntrySize) {
			throw new IllegalStateException("Expected size " + entrySize + " but was "
			...
			if (childMain != null) {
				if (childMain._parentPageNumber != null) {
					if ((int) childMain._parentPageNumber != dpMain._pageNumber) {
						throw new IllegalStateException("Child's parent is incorrect " + childMain);
					}
					boolean expectTail = ((int) subPageNumber == childTailPageNumber);
					if (expectTail != childMain._tail) {
						throw new IllegalStateException("Child tail status incorrect " + childMain);
					}
				}
				Entry lastEntry = childMain.getExtra()._entryView.getLast();
				if (e.compareTo(lastEntry) != 0) {
					throw new IllegalStateException("Invalid entry " + e + " but child is "
						+ lastEntry);
				}
			}
		}
	...
	private void validatePeers(DataPageMain dpMain) throws IOException{
		...
		if (prevMain != null) {
			if ((int) prevMain._nextPageNumber != dpMain._pageNumber) {
				throw new IllegalStateException("Prev page " + prevMain + " does not ref " + dpMain);
			}
		...
		if (nextMain != null) {
			if ((int) nextMain._prevPageNumber != dpMain._pageNumber) {
				throw new IllegalStateException("Next page " + nextMain + " does not ref " + dpMain);
			}
	...
	private void validatePeerStatus(DataPageMain dpMain, DataPageMain peerMain) throws IOException{
		if (dpMain._leaf != peerMain._leaf) {
			throw new IllegalStateException("Mismatched peer status " + dpMain._leaf + " "
				+ peerMain._leaf);
		}
		if (!dpMain._leaf) {
			if ((dpMain._parentPageNumber != null) && (peerMain._parentPageNumber != null)
				...
				throw new IllegalStateException("Mismatched node parents "
					+ dpMain._parentPageNumber + " " + peerMain._parentPageNumber);
			}
		}
	}
	 ...
	 * 
	 * @param rtn
	 *            the StringBuilder to update
	 * @param dpMain
	 *            the index page to dump
	 */
	...
	private class DataPageMain {
		...
		@Override
		public String toString(){
			return (_leaf ? "Leaf" : "Node") + "DPMain[" + _pageNumber + "] " + _prevPageNumber
				+ ", " + _nextPageNumber + ", (" + _childTailPageNumber + ")";
		}
	}
	
	/**
	 * Keeps track of the extra info for an index page. This info (if unmodified) may be re-read
	 * from disk as necessary.
	 */
	private static class DataPageExtra {
		/**
		 * sorted collection of index entries. this is kept in a list instead of a SortedSet because
		 * the SortedSet has lame traversal utilities
		 */
	 ...
	 */
	public static final class CacheDataPage extends Index.DataPage {
	...
	/**
	 * A view of an index page's entries which combines the normal entries and tail entry into one
	 * collection.
	 */
	private static class EntryListView extends AbstractList<Entry> implements RandomAccess {
		...
		private EntryListView(DataPageMain main, DataPageExtra extra) throws IOException{
			if (main.hasChildTail()) {
				_childTailEntry =
					main.getChildTailPage().getExtra()._entryView.getLast().asNodeEntry(
		...
		public Entry get(int idx){
			return (isCurrentChildTailIndex(idx) ? _childTailEntry : getEntries().get(idx));
		}
		
		@Override
		public Entry set(int idx, Entry newEntry){
			return (isCurrentChildTailIndex(idx) ? setChildTailEntry(newEntry) : getEntries().set(
				idx, newEntry));
		}
		...
		public Entry getLast(){
			return (hasChildTail() ? _childTailEntry : (!getEntries().isEmpty() ? getEntries().get(
				getEntries().size() - 1) : null));
		}
