...
public final class TempPageHolder {
	...
	/**
	 * the last "modification" count of the buffer that this holder observed. this is tracked so
	 * that the page data can be re-read if the underlying buffer has been discarded since the last
	 * page read
	 */
	 ...
	 * 
	 * @param type
	 *            the type of reference desired for any create page buffers
	 */
	...
	/**
	 * @return the page for the current page number, reading as necessary, position and limit are
	 *         unchanged
	 */
	...
	private ByteBuffer setPage(PageChannel pageChannel, int pageNumber, boolean rewind)
		throws IOException{
	...
	/**
	 * Forces any current page data to be disregarded (any <code>getPage</code>/<code>setPage</code>
	 ...
	 * <code>getPage</code>/<code>setPage</code> call must reload page data) and is not the given
