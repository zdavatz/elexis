 ...
 * {@link #apply(Iterable)} method to create an Iterable over a table which returns only rows
...
public abstract class RowFilter {
	...
	/**
	 * Returns {@code true} if the given table row matches the Filter criteria, {@code false}
	 * otherwise.
	 * 
	 * @param row
	 *            current row to test for inclusion in the filter
	 */
	 ...
	 *            target if {@code ObjectUtil.equals(rowPattern.get(s), row.get(s))} for all column
	 *            names in the pattern map.
	 * @return a filter which matches table rows which match the values in the row pattern
	 */
	 ...
	 * @param valuePattern
	 *            value to be matched. A table row will match the target if {@code
	 *            ObjectUtil.equals(valuePattern, row.get(columnPattern.getName()))}.
	...
	/**
	 * Creates a filter which inverts the sense of the given filter (rows which are matched by the
	 * given filter will not be matched by the returned filter, and vice versa).
	 * 
	 * @param filter
	 ...
	 * 
	 * @param rowFilter
	 *            the filter criteria, may be {@code null}
	 * @param iterable
	 *            row iterable to filter
	 * 
	...
	private class FilterIterable implements Iterable<Map<String, Object>> {
		...
		/**
		 * Returns an iterator which iterates through the rows of the underlying iterable, returning
		 * only rows for which the {@link RowFilter#matches} method returns {@code true}
		 */
		public Iterator<Map<String, Object>> iterator(){
			return new Iterator<Map<String, Object>>() {
				private final Iterator<Map<String, Object>> _iter = _iterable.iterator();
