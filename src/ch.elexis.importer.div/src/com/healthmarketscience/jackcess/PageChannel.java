...
public class PageChannel implements Channel, Flushable {
	...
	/**
	 * invalid page header, used when deallocating old pages. data pages generally have 4
	 * interesting bytes at the beginning which we want to reset.
	 */
	private static final byte[] INVALID_PAGE_BYTE_HEADER = new byte[] {
		PageTypes.INVALID, (byte) 0, (byte) 0, (byte) 0
	};
	...
	/**
	 * buffer used when deallocating old pages. data pages generally have 4 interesting bytes at the
	 * beginning which we want to reset.
	 */
	...
	public void initialize(Database database) throws IOException{
		...
		_globalUsageMap =
			UsageMap.read(database, PAGE_GLOBAL_USAGE_MAP, ROW_GLOBAL_USAGE_MAP, true);
	}
	...
	public void readPage(ByteBuffer buffer, int pageNumber) throws IOException{
		...
		if (bytesRead != getFormat().PAGE_SIZE) {
			throw new IOException("Failed attempting to read " + getFormat().PAGE_SIZE
				+ " bytes from page " + pageNumber + ", only read " + bytesRead);
		}
	}
	...
	public int writeNewPage(ByteBuffer page) throws IOException{
		...
		if ((size % getFormat().PAGE_SIZE) != 0L) {
			throw new IOException("Database corrupted, file size " + size
				+ " is not multiple of page size " + getFormat().PAGE_SIZE);
		}
	...
	/**
	 * Allocates a new page in the database. Data in the page is undefined until it is written in a
	 * call to {@link #writePage(ByteBuffer,int)}.
	 */
	...
	/**
	 * @return A newly-allocated buffer of the given size and LITTLE_ENDIAN byte order
	 */
	...
	/**
	 * @return a duplicate of the current buffer narrowed to the given position and limit. mark will
	 *         be set at the current position.
	 */
	public static ByteBuffer narrowBuffer(ByteBuffer buffer, int position, int limit){
		return (ByteBuffer) buffer.duplicate().order(buffer.order()).clear().limit(limit).position(
			position).mark();
	}
