...
public class Column implements Comparable<Column> {
	...
	/**
	 * Meaningless placeholder object for inserting values in an autonumber column. it is not
	 * required that this value be used (any passed in value is ignored), but using this placeholder
	 * may make code more obvious.
	 */
	public static final Object AUTO_NUMBER = "<AUTO_NUMBER>";
	
	/**
	 * Meaningless placeholder object for updating rows which indicates that a given column should
	...
	/**
	 * Access starts counting dates at Jan 1, 1900. Java starts counting at Jan 1, 1970. This is the
	 * # of millis between them for conversion.
	 */
	 ...
	 *            owning table
	 * @param buffer
	 *            Buffer containing column definition
	 * @param offset
	 *            Offset in the buffer at which the column definition starts
	 */
	public Column(Table table, ByteBuffer buffer, int offset) throws IOException{
		_table = table;
		...
		_compressedUnicode =
			((buffer.get(offset + getFormat().OFFSET_COLUMN_COMPRESSED_UNICODE) & 1) == 1);
		
		if (_variableLength) {
			_varLenTableIndex =
				buffer.getShort(offset + getFormat().OFFSET_COLUMN_VARIABLE_TABLE_INDEX);
		} else {
			_fixedDataOffset =
	...
	/**
	 * Also sets the length and the variable length flag, inferred from the type. For types with
	 * scale/precision, sets the scale and precision to default values.
	 */
	...
	/**
	 * Returns the AutoNumberGenerator for this column if this is an autonumber column, {@code null}
	 * otherwise.
	 */
	 ...
	 *             if this column definition is invalid.
	 */
	public void validate(JetFormat format){
		...
		if (getType().getHasScalePrecision()) {
			if (!getType().isValidScale(getScale())) {
				throw new IllegalArgumentException("Scale must be from " + getType().getMinScale()
					+ " to " + getType().getMaxScale() + " inclusive");
			}
			if (!getType().isValidPrecision(getPrecision())) {
				throw new IllegalArgumentException("Precision must be from "
					+ getType().getMinPrecision() + " to " + getType().getMaxPrecision()
					+ " inclusive");
			}
		}
		...
		if (isCompressedUnicode()) {
			if ((getType() != DataType.TEXT) && (getType() != DataType.MEMO)) {
				throw new IllegalArgumentException(
	...
	/**
	 * @param lvalDefinition
	 *            Column value that points to an LVAL record
	 ...
	 * 
	 * @param buffer
	 *            Column value that points to currency data
	...
	private void writeNumericValue(ByteBuffer buffer, Object value) throws IOException{
		try {
			...
			if (intValBytes.length < maxByteLen) {
				...
				System.arraycopy(intValBytes, 0, tmpBytes, (maxByteLen - intValBytes.length),
					intValBytes.length);
	...
	private double toDateDouble(Object value){
		...
		long time =
			((value instanceof Date) ? ((Date) value).getTime()
	 ...
	 * Writes a GUID value.
	 */
	private void writeGUIDValue(ByteBuffer buffer, Object value, ByteOrder order)
		throws IOException{
	 ...
	 * @return A buffer containing the LVAL definition and (possibly) the column value (unless
	 *         written to other pages)
	 */
	public ByteBuffer writeLongValue(byte[] value, int remainingRowLength) throws IOException{
		if (value.length > getType().getMaxSize()) {
			throw new IOException("value too big for column, max " + getType().getMaxSize()
				+ ", got " + value.length);
		}
		...
		if (type == LONG_VALUE_TYPE_THIS_PAGE) {
			...
			case LONG_VALUE_TYPE_OTHER_PAGES:
				...
				while (remainingLen > 0) {
					...
					int chunkLength =
						Math.min(getFormat().MAX_LONG_VALUE_ROW_SIZE - 4, remainingLen);
					...
					if (chunkLength < remainingLen) {
						...
						lvalBufferH.clear();
						nextLvalPage =
							getLongValuePage(getFormat().MAX_LONG_VALUE_ROW_SIZE, lvalBufferH);
					...
					byte lvalRow =
						(byte) Table.addDataPageRow(lvalPage, chunkLength + 4, getFormat(), 0);
	...
	public ByteBuffer writeFixedLengthField(Object obj, ByteOrder order) throws IOException{
		...
		case UNKNOWN_11:
			...
			if (bytes.length != getLength()) {
				throw new IOException("Invalid fixed size binary data, size " + getLength()
					+ ", got " + bytes.length);
			}
			buffer.put(bytes);
			break;
	...
	private String decodeTextValue(byte[] data) throws IOException{
		try {
			...
			boolean isCompressed =
				((data.length > 1) && (data[0] == TEXT_COMPRESSION_HEADER[0]) && (data[1] == TEXT_COMPRESSION_HEADER[1]));
			...
			if (isCompressed) {
				...
				while (dataEnd < data.length) {
					if (data[dataEnd] == (byte) 0x00) {
						...
						decodeTextSegment(data, dataStart, dataEnd, inCompressedMode, expander,
							textBuf);
	...
	/**
	 * Decodes a segnment of a text value into the given buffer according to the given status of the
	 * segment (compressed/uncompressed).
	 */
	private void decodeTextSegment(byte[] data, int dataStart, int dataEnd,
		boolean inCompressedMode, Expand expander, StringBuilder textBuf)
	...
	/**
	 * @param textBytes
	 *            bytes of text to decode
	 * @return the decoded string
	 */
	private static CharBuffer decodeUncompressedText(byte[] textBytes, int startPos, int length,
		Charset charset){
	...
	private ByteBuffer encodeTextValue(Object obj, int minChars, int maxChars,
		...
		if ((text.length() > maxChars) || (text.length() < minChars)) {
			throw new IOException("Text is wrong length for " + getType() + " column, max "
	...
	/**
	 * Returns {@code true} if the given text can be compressed using simple ASCII encoding, {@code
	 * false} otherwise.
	 */
	...
	public String toString(){
		...
		rtn.append("\n\tType: 0x" + Integer.toHexString(_type.getValue()) + " (" + _type + ")");
	...
	/**
	 * @return an appropriate BigDecimal representation of the given object. <code>null</code> is
	 *         returned as 0 and Numbers are converted using their double representation.
	 */
	...
	/**
	 * @return an appropriate Number representation of the given object. <code>null</code> is
	 *         returned as 0 and Strings are parsed as Doubles.
	 */
	 ...
	 * Useful for wrapping unparseable data for re-writing.
	 */
	...
	/**
	 * Returs {@code true} if the given value is "raw" column data, {@code false} otherwise.
	 */
	...
	/**
	 * Date subclass which stashes the original date bits, in case we attempt to re-write the value
	...
	public abstract class AutoNumberGenerator {
		...
		/**
		 * Returns the last autonumber generated by this generator. Only valid after a call to
		 * {@link Table#addRow}, otherwise undefined.
		 */
		 ...
		 * <p>
		 * <i>Warning, calling this externally will result in this value being "lost" for the
		 * table.</i>
		 */
