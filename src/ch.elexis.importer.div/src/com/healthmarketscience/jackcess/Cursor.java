...
/**
 * Manages iteration for a Table. Different cursors provide different methods of traversing a table.
 * Cursors should be fairly robust in the face of table modification during traversal (although
 * depending on how the table is traversed, row updates may or may not be seen). Multiple cursors
 * may traverse the same table simultaneously.
 * <p>
 ...
 * construction, see {@link CursorBuilder}.
 * <p>
 * Is not thread-safe.
 * 
...
public abstract class Cursor implements Iterable<Map<String, Object>> {
	 ...
	 * 
	 * @param table
	 *            the table over which this cursor will traverse
	 */
	 ...
	 *            lookups
	 * @param startRow
	 *            the first row of data for the cursor (inclusive), or {@code null} for the first
	 *            entry
	 * @param endRow
	 *            the last row of data for the cursor (inclusive), or {@code null} for the last
	 *            entry
	 */
	public static Cursor createIndexCursor(Table table, Index index, Object[] startRow,
		Object[] endRow) throws IOException{
	 ...
	 *            lookups
	 * @param startRow
	 *            the first row of data for the cursor, or {@code null} for the first entry
	 * @param startInclusive
	 *            whether or not startRow is inclusive or exclusive
	 * @param endRow
	 *            the last row of data for the cursor, or {@code null} for the last entry
	 * @param endInclusive
	 *            whether or not endRow is inclusive or exclusive
	 */
	public static Cursor createIndexCursor(Table table, Index index, Object[] startRow,
		boolean startInclusive, Object[] endRow, boolean endInclusive) throws IOException{
		if (table != index.getTable()) {
			throw new IllegalArgumentException("Given index is not for given table: " + index
				+ ", " + table);
		}
		if (!table.getFormat().INDEXES_SUPPORTED) {
			throw new IllegalArgumentException("JetFormat " + table.getFormat()
				+ " does not currently support index lookups");
		}
		return new IndexCursor(table, index, index.cursor(startRow, startInclusive, endRow,
			endInclusive));
	}
	
	/**
	 * Convenience method for finding a specific row in a table which matches a given row "pattern".
	 * See {@link #findRow(Map)} for details on the rowPattern.
	 * 
	 * @param table
	 *            the table to search
	 * @param rowPattern
	...
	/**
	 * Convenience method for finding a specific row in a table which matches a given row "pattern".
	 * See {@link #findRow(Column,Object)} for details on the pattern.
	 * <p>
	 * Note, a {@code null} result value is ambiguous in that it could imply no match or a matching
	 * row with {@code null} for the desired value. If distinguishing this situation is important,
	 * you will need to use a Cursor directly instead of this convenience method.
	 * 
	 * @param table
	 *            the table to search
	 * @param column
	 *            column whose value should be returned
	 * @param columnPattern
	 *            column being matched by the valuePattern
	 * @param valuePattern
	...
	public static Object findValue(Table table, Column column, Column columnPattern,
		Object valuePattern) throws IOException{
		Cursor cursor = createCursor(table);
	...
	/**
	 * Convenience method for finding a specific row in an indexed table which matches a given row
	 * "pattern". See {@link #findRow(Map)} for details on the rowPattern.
	 * 
	 * @param table
	 *            the table to search
	 * @param index
	 *            index to assist the search
	 * @param rowPattern
	 *            pattern to be used to find the row
	 * @return the matching row or {@code null} if a match could not be found.
	 */
	public static Map<String, Object> findRow(Table table, Index index,
		Map<String, Object> rowPattern) throws IOException{
	...
	/**
	 * Returns the current state of the cursor which can be restored at a future point in time by a
	 * call to {@link #restoreSavepoint}.
	 * <p>
	...
	public void restoreSavepoint(Savepoint savepoint) throws IOException{
		if (!_id.equals(savepoint.getCursorId())) {
			throw new IllegalArgumentException("Savepoint " + savepoint
				+ " is not valid for this cursor with id " + _id);
		}
	...
	/**
	 * Returns {@code true} if the cursor is currently positioned before the first row, {@code
	 * false} otherwise.
	 */
	...
	/**
	 * Returns {@code true} if the cursor is currently positioned after the last row, {@code false}
	 * otherwise.
	 */
	...
	/**
	 * Returns {@code true} if the row at which the cursor is currently positioned is deleted,
	 * {@code false} otherwise (including invalid rows).
	 */
	...
	/**
	 * Returns an Iterable whose iterator() method calls <code>afterLast</code> on this cursor and
	...
	/**
	 * Returns an Iterable whose iterator() method calls <code>afterLast</code> on this table and
	 * returns an unmodifiable Iterator which will iterate through all the rows of this table in
	 * reverse order, returning only the given columns. Use of the Iterator follows the same
	 * restrictions as a call to <code>getPreviousRow</code>.
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	...
	/**
	 * Calls <code>beforeFirst</code> on this cursor and returns an unmodifiable Iterator which will
	...
	/**
	 * Returns an Iterable whose iterator() method returns the result of a call to
	 * {@link #iterator(Collection)}
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	...
	/**
	 * Calls <code>beforeFirst</code> on this table and returns an unmodifiable Iterator which will
	 ...
	 * 
	 * @return The next row in this table (Column name -> Column value), or {@code null} if no next
	 *         row is found
	 */
	 ...
	 * 
	 * @return The previous row in this table (Column name -> Column value), or {@code null} if no
	 ...
	 * @param columnNames
	 *            Only column names in this collection will be returned
	 * @return another row in this table (Column name -> Column value), where "next" may be
	 *         backwards if moveForward is {@code false}, or {@code null} if there is not another
	 *         row in the given direction.
	 */
	private Map<String, Object> getAnotherRow(Collection<String> columnNames, boolean moveForward)
		throws IOException{
	 ...
	 * 
	 * @return {@code true} if another valid row was found in the given direction, {@code false}
	 *         otherwise
	 */
	...
	/**
	 * Restores a current and previous position for the cursor if the given positions are different
	 ...
	 * This may be more efficient on some cursors than others. If a match is not found (or an
	 * exception is thrown), the cursor is restored to its previous state.
	 * 
	 * @param columnPattern
	 *            column from the table for this cursor which is being matched by the valuePattern
	 * @param valuePattern
	 *            value which is equal to the corresponding value in the matched row
	 * @return {@code true} if a valid row was found with the given value, {@code false} if no row
	 *         was found
	 */
	...
	/**
	 * Moves to the first row (as defined by the cursor) where the given columns have the given
	 * values. This may be more efficient on some cursors than others. If a match is not found (or
	 * an exception is thrown), the cursor is restored to its previous state.
	 * 
	 * @param rowPattern
	 *            column names and values which must be equal to the corresponding values in the
	 *            matched row
	 * @return {@code true} if a valid row was found with the given values, {@code false} if no row
	 *         was found
	 */
	...
	/**
	 * Moves to the first row (as defined by the cursor) where the given column has the given value.
	 * Caller manages save/restore on failure.
	 * <p>
	...
	/**
	 * Moves to the first row (as defined by the cursor) where the given columns have the given
	 * values. Caller manages save/restore on failure.
	 * <p>
	 * Default implementation scans the table from beginning to end.
	 * 
	 * @param rowPattern
	...
	/**
	 * Returns {@code true} if this cursor is up-to-date with respect to the relevant table and
	 * related table objects, {@code false} otherwise.
	 */
	...
	public String toString(){
		return getClass().getSimpleName() + " CurPosition " + _curPos + ", PrevPosition "
			+ _prevPos;
	}
	
	/**
	 * Finds the next non-deleted row after the given row (as defined by this cursor) and returns
	 * the id of the row, where "next" may be backwards if moveForward is {@code false}. If there
	 * are no more rows, the returned rowId should equal the value returned by
	 * {@link #getLastPosition} if moving forward and {@link #getFirstPosition} if moving backward.
	 */
	protected abstract Position findAnotherPosition(RowState rowState, Position curPos,
	...
	private static final class TableScanCursor extends Cursor {
		...
		/**
		 * Handles moving the table scan cursor in a given direction. Separates cursor logic from
		 * value storage.
		 */
	...
	private static final class IndexCursor extends Cursor {
		...
		private IndexCursor(Table table, Index index, Index.EntryCursor entryCursor)
			throws IOException{
			super(new Id(table, index), table, new IndexPosition(entryCursor.getFirstEntry()),
		...
		protected boolean findRowImpl(Column columnPattern, Object valuePattern) throws IOException{
			Object[] rowValues =
				_entryCursor.getIndex().constructIndexRow(columnPattern.getName(), valuePattern);
		...
		/**
		 * Handles moving the table index cursor in a given direction. Separates cursor logic from
		 * value storage.
		 */
	...
	public static final class Id {
		...
		public boolean equals(Object o){
			return ((this == o) || ((o != null) && (getClass() == o.getClass())
				&& ObjectUtils.equals(_tableName, ((Id) o)._tableName) && ObjectUtils.equals(
				_indexName, ((Id) o)._indexName)));
		}
	...
	public static final class Savepoint {
		...
		@Override
		public String toString(){
			return getClass().getSimpleName() + " " + _cursorId + " CurPosition " + _curPos
	...
	public static abstract class Position {
		...
		/**
		 * Returns {@code true} if the subclass specific info in a Position is equal, {@code false}
		 * otherwise.
		 * 
		 * @param o
